...
class DiscordPoster {
  ...
  async postReport(category, tweet, replySuggestions) {
    ...
    const tweetLink = `[Open in X ↗](${tweet.url})`;
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '❤️ Likes', value: tweet.metrics.likes.toLocaleString(), inline: true },
        { name: '🔄 Retweets', value: tweet.metrics.retweets.toLocaleString(), inline: true },
        { name: '💬 Replies', value: tweet.metrics.replies.toLocaleString(), inline: true }
      )
  ...
  async postHeader(hour, bangkokTime) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle(`📊 Viral Tweets Report`)
      .setDescription(`Top tweets from the last hour\n🕐 Bangkok: ${bangkokTime} | UTC: ${hour}:00`)
      .setTimestamp();

    await channel.send({ embeds: [embed] });
    await channel.send('─'.repeat(40)); // Separator
  }
  ...
  async postNoTweetsMessage() {
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle('⚠️ No Viral Tweets Found')
